...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (userLogin && token) {
        ...
        // Скрываем защищенные пункты меню, но оставляем кнопку изменения темы видимой
        const profileMenu = document.querySelector('.profile-menu');
        if (profileMenu) {
            const protectedLinks = profileMenu.querySelectorAll('a:not(.auth-menu a):not(.profile-theme-toggle)');
            protectedLinks.forEach(link => {
                link.style.display = 'none';
            });
            
            // Убеждаемся, что кнопка изменения темы видна
            const themeToggle = profileMenu.querySelector('.profile-theme-toggle');
            if (themeToggle && !authMenu.contains(themeToggle)) {
